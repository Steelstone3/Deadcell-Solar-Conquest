use crate::components::map::planet::{PLANET_CLOSEST_DISTANCE_TO_STAR, Planet};
use crate::events::spawn_animated_sprite_event::SpawnAnimatedSprite;
...
pub fn spawn_resource_planets(
    ...
    for star_query in star_queries.iter() {
        ...
        for _ in 0..number_of_planets {
            ...
            let mut transform = star_query
                .transform
                .with_rotation(Quat::from_rotation_z(angle.to_radians()));

            transform.translation +=
                transform.up() * PLANET_CLOSEST_DISTANCE_TO_STAR * rng.gen_range(1.0..3.0)
                    + (cmp::max(
